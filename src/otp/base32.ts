 ...
 *
 * pwa-authenticator modifications:
 *   - translated to typescript
 *   - less strict padding requirements (will decode with/without padding)
 */
 ...
 */
export function encode(data: ArrayBuffer, pad: boolean = false): string {
 ...
 */
export function encodeHex(data: ArrayBuffer, pad: boolean = false): string {
...
export function decode(str: string): ArrayBuffer {
	return _decode(str, TABLE)
}

/**
 * Decode a base32hex string.
 */
export function decodeHex(str: string): ArrayBuffer {
	return _decode(str, TABLE_HEX)
}

/**
 * Decode a base32 string.
 */
function _decode(str: string, table: number[]): ArrayBuffer {
	const data = new Uint8Array(((str.length * 5 + 7) / 8) | 0)
